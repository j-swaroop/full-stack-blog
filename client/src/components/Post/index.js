...
const Post = () => {
    ...
    const getPost = () => axios.get(`http://localhost:3001/posts/byId/${id}`).then((response) => {
        setPostObj(response.data)
    })

    const getComments = () => axios.get(`http://localhost:3001/comments/${id}`).then((response) => {
    ...
    const onEditPostTitle = async (title) => {
        ...
        if (newTitle === null){
            ...
            await axios.put(`http://localhost:3001/posts/title`, {newTitle: newTitle, id: id}, {
                headers: {
                    accessToken: Cookies.get('jwt_token')
                }
            })
    ...
    const onEditPostText = async (text) => {
        ...
        if (newText !== null){
            await axios.put(`http://localhost:3001/posts/postText`, {newText: newText, id: id}, {
                headers: {
                    accessToken: Cookies.get('jwt_token')
                }
            })
